...
transformed parameters{
  vector[n_Ap] logitp_Ap = to_vector(alpha_y[1, year_Ap]) + alpha_p[plot_Ap] + X_Ap * beta_Ap;
  vector[n_Er] logitp_Er = to_vector(alpha_y[2, year_Er]) + alpha_p[plot_Er] + X_Er * beta_Er;
  vector[n_Ev] logitp_Ev = to_vector(alpha_y[3, year_Ev]) + alpha_p[plot_Ev] + X_Ev * beta_Ev;
  vector[n_Fs] logitp_Fs = to_vector(alpha_y[4, year_Fs]) + alpha_p[plot_Fs] + X_Fs * beta_Fs;
  vector[n_Pa] logitp_Pa = to_vector(alpha_y[5, year_Pa]) + alpha_p[plot_Pa] + X_Pa * beta_Pa;
  vector[n_Pu] logitp_Pu = to_vector(alpha_y[6, year_Pu]) + alpha_p[plot_Pu] + X_Pu * beta_Pu;
  vector[n_Ps] logitp_Ps = to_vector(alpha_y[7, year_Ps]) + alpha_p[plot_Ps] + X_Ps * beta_Ps;
}
model {
  beta_Ap ~ normal(0,1);
  beta_Er ~ normal(0,1);
