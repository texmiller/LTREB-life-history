data {
  int<lower=0> n_obs;                   // plant-age observations
  ...
  int<lower=1,upper=n_cohorts> cohort[n_obs];
  int<lower=1> n_years;
...
transformed parameters {
  ...
  for (i in 1:n_obs) {
    logit_h[i] = intercept + age_eff[species[i]][endo[i]+1,age[i]]
               ...
               + z_c[species[i],cohort[i]]*sigma_cohort
...
generated quantities {
  ...
  for (i in 1:n_ind) {
    ...
    while (has_flowered == 0 && a <= n_ages) {
      ...
      real h_a = inv_logit(intercept+age_eff[s][e,a]+
                            ...
                            z_c[s,cohort_ind[i]]*sigma_cohort+
                            z_y[s,yr]*sigma_year);
  ...
  //(3) derive year and plot effects from non-centered parameters
  matrix[n_spp,n_cohorts] alpha_c;
  matrix[n_spp,n_years] alpha_y;
  for(s in 1:n_spp){
    for(c in 1:n_cohorts){
      alpha_c[s,c]=z_c[s,c]*sigma_cohort;
    }
    for(y in 1:n_years){
      alpha_y[s,y]=z_y[s,y]*sigma_year;
    }
  }
